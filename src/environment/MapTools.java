    ...
    public static double calculateMapMetric1(Environment env) {
        ...
        for (int i = 1; i < (env.getColumns() - 1); i++) {
            for (int j = 1; j < (env.getRows() - 1); j++) {
                if (env.statusAt(i, j) == env.statusAt(i - 1, j)) {
                    t_s++;
                } else {
                    t_d++;    // NOTE statusAt changes as exploration unfolds
                }
                if (env.statusAt(i, j) == env.statusAt(i, j - 1)) {
                    t_s++;
                } else {
                    t_d++;    // so as is, only measure metric before start of exploration
                }
                if (env.statusAt(i, j) == env.statusAt(i + 1, j)) {
                    t_s++;
                } else {
